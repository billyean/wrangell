...
public class ParsedKeywordBidSerializerTest {
    SimpleKeywordLookupService service = new SimpleKeywordLookupService();

    @Before
    public void setUp() throws JsonProcessingException {
        int agid1 = 123;
        String keywordBidJson1 = "[{\"keywords\":[\"1~facial tissue\",\"2~cloth\",\"3~paper tissue\"]," +
                    " \"matchType\":\"phrase\",\"cpc\":0.5}," +
                "{\"keywords\":[\"4~chocolate cream\", \"5~nuts\", \"6~cereal crunch\"], " +
                "   \"matchType\":\"broad\",\"cpc\":0.5}," +
                "{\"keywords\":[\"7~facial tissue\", \"8~nuts\", \"9~cereal crunch\"], " +
                "   \"matchType\":\"broad\",\"cpc\":0.75}," +
                "{\"keywords\":[\"10~facial tissue paper\"], \"matchType\":\"exact\",\"cpc\":0.2}]";

        int agid2 = 345;
        String keywordBidJson2 = "[{\"keywords\":[\"1~facial tissue\",\"2~cloth\",\"3~paper tissue\"], " +
                "\"matchType\":\"phrase\",\"cpc\":0.5}," +
                "{\"keywords\":[\"10~facial tissue paper\"], \"matchType\":\"exact\",\"cpc\":0.2}," +
                "{\"keywords\":[\"11~facial tissue paper\"], \"matchType\":\"exact\",\"cpc\":0.9}]";

        int agid3 = 257686;
        String keywordBidJson3 = "[{\"keywords\":[\"4211930~1 cup tupperware\",\"4211937~12 cup\",\"4211939~14 cup measure\"," +
        ...
        int agid4 = 257686;
        String keywordBidJson4 = "[{\"keywords\":[\"3442058~fox run\"],\"matchType\":\"exact\",\"cpc\":0.3}," +
    ...
    public void testSerialization() {
        Set<ParsedKeywordBid> original =service.search(WMT, 257686);
        ParsedKeywordBidSerializer serializer = new ParsedKeywordBidSerializer();
        byte[] bytes = serializer.serialize(original);

        /**
         * Small time elapsed test here:
         * Deserialize 1000,000 times cost 68699 ms.
         *     Set<ParsedKeywordBid> actual = new HashSet<>();
         *     long time = System.currentTimeMillis();
         *     for (int i =0; i < 1000000; i++) {
         *         actual = serializer.deserialize(bytes);
         *     }
         *     System.out.println("Deserialize 1000,000 times costs " + (System.currentTimeMillis() - time) + " ms.");
         */
        Set<ParsedKeywordBid> actual = serializer.deserialize(bytes);
        assertEquals(original.size(), actual.size());

        for (ParsedKeywordBid parsedKeywordBid : original) {
            assertTrue(String.format("%s is not found in actual", parsedKeywordBid), actual.contains(parsedKeywordBid));
        }
    }
}
